...
def eval_perturbation_ratio(eval_dataloader, perturb_dataloader, model):
    ...
    for batch, perturb_batch in tqdm(zip(eval_dataloader, perturb_dataloader)):
        ...
        truth_ratio = torch.exp(gt_loss_per_token.float() - perturb_loss_per_token.float().mean(-1))
        ...
        num_token_gt = dict(zip(indices.cpu().numpy().tolist(), num_token_gt.cpu().float().numpy().tolist()))
        num_token_perturb = dict(zip(indices.cpu().numpy().tolist(), num_token_perturb.cpu().float().numpy().tolist()))
...
def get_all_evals(cfg, model, tokenizer, eval_task, eval_dataloader, base_eval_dataloader, perturb_dataloader, normalize_gt=False):
    ...
    for batch in tqdm(eval_dataloader):
        ...
        eval_logs['num_token_gt'].update(dict(zip(indices.cpu().numpy().tolist(), num_token_gt.cpu().float().numpy().tolist())))
...
@hydra.main(version_base=None, config_path="config", config_name="evaluate_util.yaml")
def main(cfg):
    ...
    for attempt in range(3):
        try:
            # Log device map
            print(f"Attempt {attempt + 1}: Loading model with device_map: {device_map}")
            ...
            print(f"Error loading model at attempt {attempt + 1}: {e}")
            continue
        else:
            break
    else:
    ...
    if model is None:
        raise RuntimeError("Model could not be loaded after 3 attempts.")
...
def run_generation(cfg, batch, model, tokenizer):
    ...
    split_symbol = "\n" if cfg.model_family == 'phi' else 'Answer: '
