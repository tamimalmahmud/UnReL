...
from scipy.stats import sem, hmean, ks_2samp, wasserstein_distance, entropy
...
def get_forget_quality(unlearn_result, retain_result):
    ...
    js_divergence = jensenshannon(unlearn_truth_ratio, retain_truth_ratio)**2

    # Wasserstein Distance
    wasserstein_dist = wasserstein_distance(unlearn_truth_ratio, retain_truth_ratio)

    # Entropy-based Measures
    entropy_unlearn = entropy(unlearn_truth_ratio)
    entropy_retain = entropy(retain_truth_ratio)
    entropy_diff = abs(entropy_unlearn - entropy_retain)
    ...
    return {
        ...
        'Entropy Unlearn': entropy_unlearn,
        'Entropy Retain': entropy_retain,
        'Entropy Difference': entropy_diff
    }
...
def main(cfg):
    ...
    model_utility.update({
        'Forget Quality': forget_quality['Forget Quality'],
        #'KS Test PVal Forget': forget_quality['KS Test PVal Forget'],
        'KS Test Forget': forget_quality['KS Test Forget'],
        'Jensen-Shannon Divergence': forget_quality['Jensen-Shannon Divergence'],
#        'Wasserstein Distance': forget_quality['Wasserstein Distance'],
        'Entropy Difference': forget_quality['Entropy Difference']
    })
